...
class ProjectManager {
     ...
     */
    constructor(rootPath, inMemoryFileSystem, updater, traceModuleResolution, logger = new logging_1.NoopLogger()) {
        this.logger = logger;
        /**
        ...
        for (const configType of ['js', 'ts']) {
            ...
            const config = new ProjectConfiguration(this.inMemoryFs, documentRegistry, trimmedRootPath, this.versions, '', tsConfig, this.traceModuleResolution, this.logger);
        ...
        this.subscriptions.add(rxjs_1.Observable.fromEvent(inMemoryFileSystem, 'add', Array.of)
            ...
            let dir = util_1.toUnixPath(filePath);
            ...
            const configs = this.configs[configType];
            configs.set(dir, new ProjectConfiguration(this.inMemoryFs, documentRegistry, dir, this.versions, filePath, undefined, this.traceModuleResolution, this.logger));
        ...
        return tracing_1.traceObservable('Ensure own files', childOf, span => {
            if (!this.ensuredOwnFiles) {
                this.ensuredOwnFiles = this.updater.ensureStructure(span)
                    .concat(rxjs_1.Observable.defer(() => util_1.observableFromIterable(this.inMemoryFs.uris())))
                    .filter(uri => !uri.includes('/node_modules/') && util_1.isJSTSFile(uri) || util_1.isConfigFile(uri) || util_1.isPackageJsonFile(uri))
                    .mergeMap(uri => this.updater.ensure(uri))
                    .do(lodash_1.noop, err => {
                    this.ensuredOwnFiles = undefined;
                })
    ...
    resolveReferencedFiles(uri, span = new opentracing_1.Span()) {
        ...
        observable = this.updater.ensure(uri)
            ...
            .map(filePath => util_1.path2uri(filePath))
            .do(lodash_1.noop, err => {
            this.referencedFiles.delete(uri);
        })
            .publishReplay()
    ...
    getChildConfigurations(uri) {
        ...
        return iterare_1.default(this.configs.ts).concat(this.configs.js)
...
class ProjectConfiguration {
     ...
     */
    constructor(fs, documentRegistry, rootFilePath, versions, configFilePath, configContent, traceModuleResolution, logger = new logging_1.NoopLogger()) {
    ...
    ensureBasicFiles(span = new opentracing_1.Span()) {
        ...
        for (const uri of this.fs.uris()) {
            ...
            if (util_1.isGlobalTSFile(fileName) || (util_1.isDeclarationFile(fileName) && this.expectedFilePaths.has(util_1.toUnixPath(fileName)))) {
