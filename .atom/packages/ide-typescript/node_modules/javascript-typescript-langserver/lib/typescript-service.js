...
class TypeScriptService {
    ...
    workspaceDidChangeConfiguration(params) {
        lodash_2.merge(this.settings, params.settings);
    }
    /**
    ...
    _getDefinitionLocations(params, span = new opentracing_1.Span()) {
        ...
        return this.projectManager.ensureReferencedFiles(uri, undefined, undefined, span)
            ...
            return rxjs_1.Observable.from(definitions || [])
                .map((definition) => {
    ...
    _getSymbolLocationInformations(params, span = new opentracing_1.Span()) {
        ...
        return this.projectManager.ensureReferencedFiles(uri, undefined, undefined, span)
            ...
            return rxjs_1.Observable.from(configuration.getService().getDefinitionAtPosition(fileName, offset) || [])
                .mergeMap((definition) => {
    ...
    textDocumentHover(params, span = new opentracing_1.Span()) {
        return this._getHover(params, span)
            .map(hover => ({ op: 'add', path: '', value: hover }));
    }
    /**
        ...
        return this.projectManager.ensureReferencedFiles(uri, undefined, undefined, span)
            ...
            if (info.kind) {
                ...
                const modifiers = info.kindModifiers
                    .split(',')
                    .filter(mod => mod && (mod !== ts.ScriptElementKindModifier.exportedModifier
        ...
        let observable = this.isDefinitelyTyped
            ...
            if (isDefinitelyTyped) {
                ...
                return this.updater.ensureStructure(span)
                    ...
                    return this._getSymbolsInConfig(config, lodash_2.omit(params.symbol, 'package'), span);
                }));
            }
            ...
            return this.projectManager.ensureOwnFiles(span)
                ...
                if (params.symbol && params.symbol.package && params.symbol.package.name) {
                    ...
                    return util_1.observableFromIterable(this.packageManager.packageJsonUris())
                        ...
                        .mergeMap(packageJsonUri => rxjs_1.Observable.merge(lodash_2.castArray(this.projectManager.getParentConfiguration(packageJsonUri) || []), 
    ...
    workspaceXreferences(params, span = new opentracing_1.Span()) {
        const queryWithoutPackage = lodash_2.omit(params.query, 'package');
        ...
        return this.isDefinitelyTyped
            ...
            if (params.hints && params.hints.dependeePackageName) {
                return util_1.observableFromIterable(this.packageManager.packageJsonUris())
                    .filter(uri => JSON.parse(this.inMemoryFileSystem.getContent(uri)).name === params.hints.dependeePackageName)
                    .take(1)
                    .mergeMap(uri => {
            ...
            util_1.observableFromIterable(ast_1.walkMostAST(source))
                ...
                try {
                    ...
                    return rxjs_1.Observable.from(config.getService().getDefinitionAtPosition(source.fileName, node.pos + 1) || [])
                        .mergeMap(definition => {
                        const symbol = symbols_1.definitionInfoToSymbolDescriptor(definition, this.root);
    ...
    workspaceXpackages(params = {}, span = new opentracing_1.Span()) {
        return this.isDefinitelyTyped
            ...
            return this.projectManager.ensureModuleStructure(span)
                ...
                return rxjs_1.Observable.of('dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies')
                    .filter(key => !!packageJson[key])
                    .mergeMap(key => lodash_1.toPairs(packageJson[key]))
                    .map(([name, version]) => ({
                    attributes: {
                        name,
                        version
    ...
    workspaceXdependencies(params = {}, span = new opentracing_1.Span()) {
        ...
        return this.projectManager.ensureModuleStructure()
            ...
            .filter(uri => uri.includes('/package.json') && !uri.includes('/node_modules/'))
            .mergeMap(uri => this.packageManager.getPackageJson(uri))
            .mergeMap(packageJson => rxjs_1.Observable.of('dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies')
            .filter(key => !!packageJson[key])
            .mergeMap(key => lodash_1.toPairs(packageJson[key]))
            .map(([name, version]) => ({
            attributes: {
    ...
    textDocumentSignatureHelp(params, span = new opentracing_1.Span()) {
        ...
        return this.projectManager.ensureReferencedFiles(uri, undefined, undefined, span)
            ...
            const offset = ts.getPositionOfLineAndCharacter(sourceFile, params.position.line, params.position.character);
            const signatures = configuration.getService().getSignatureHelpItems(filePath, offset);
            if (!signatures) {
    ...
    _getSymbolsInConfig(config, query, childOf = new opentracing_1.Span()) {
        return tracing_1.traceObservable('Get symbols in config', childOf, span => {
            ...
            if (typeof query === 'string') {
                ...
                const queryWithoutPackage = query && lodash_2.omit(query, 'package');
                ...
                return rxjs_1.Observable.from(program.getSourceFiles())
                    .filter(sourceFile => !memfs_1.isTypeScriptLibrary(sourceFile.fileName) && !sourceFile.fileName.includes('/node_modules/'))
                    .mergeMap(sourceFile => {
                    try {
                        const tree = service.getNavigationTree(sourceFile.fileName);
                        const nodes = util_1.observableFromIterable(symbols_1.walkNavigationTree(tree))
                            .filter(({ tree, parent }) => symbols_1.navigationTreeIsSymbol(tree));
                        let matchedNodes;
                        if (!query) {
                            matchedNodes = nodes
                        ...
                        return matchedNodes
                            .map(({ score, tree, parent }) => [score, symbols_1.navigationTreeToSymbolInformation(tree, parent, sourceFile, this.root)]);
                    }
                    catch (e) {
